...
function Sidebar() {
  ...
  // Objeto que mapea nombres de sala a emojis
  const roomEmojis = {
    general: "🌎",
    tech: "💻",
    finance: "💰",
    crypto: "📈",
    MonkIA: "🤖",
    // ... Agrega más emojis para las salas que desees
  };
    ...
    return (
        ...
        {rooms.map((room, idx) => (
          <ListGroup.Item key={idx} onClick={() => joinRoom(room)} active={room == currentRoom} style={{ cursor: "pointer", display: "flex", justifyContent: "space-between" }}>
            {roomEmojis[room]} {room} {currentRoom !== room && <span className="badge rounded-pill">{user.newMessages[room]}</span>}
          </ListGroup.Item>
        ))}
      </ListGroup>
            <h2>Members</h2>
